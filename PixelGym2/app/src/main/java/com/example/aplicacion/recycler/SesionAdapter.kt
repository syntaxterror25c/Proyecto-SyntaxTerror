...
class SesionAdapter(
    ...
    inner class SesionViewHolder(private val binding: ItemSesionBinding) :
        ...
        fun bind(sesion: Sesion) {
            // Si el nombre viene de la colección 'actividades' se llama 'nombre',
            // si viene de 'sesiones' se llama 'nombre_actividad'. Cubrimos ambos:
            ...
            // Si no hay hora (es catálogo), ponemos un texto vacío o indicativo
            binding.tvHoraInicio.text = sesion.hora_inicio.ifEmpty { "--:--" }

            // CORRECCIÓN SEGURIDAD: Evita errores si capacidad es 0 (catálogo)
